...
export default async function EditProduct({ params }: Props) {
	...
	const result = await GetProduct(session?.user.id!, session?.user.session!, params.Id);
    ...
    return (
        <main className='max-w-5xl mx-auto px-5 pb-10 flex gap-6 flex-col'>
            ...
            <ProductForm type='edit' defaultValues={values.defaultValues} message={values.message} />
        </main>
    )
};